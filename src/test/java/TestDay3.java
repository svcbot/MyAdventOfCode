import day3.Day3;
import day3.Orientation;
import day3.Point;
import day3.WireSection;
import org.junit.Test;

import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

import static day3.Day3.*;
import static org.junit.Assert.*;

public class TestDay3 {
   @Test
   public void testStage1() {
      assertEquals("Test 1: ", 159, Day3.solveStage1("R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83"));
      assertEquals("Test 2: ", 135, Day3.solveStage1("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7"));
   }

   @Test
   public void testFindIntersections() {
      List<WireSection> wire1 = new ArrayList<WireSection>();
      List<WireSection> wire2 = new ArrayList<WireSection>();

      wire1.add(new WireSection(25,50,75,50));
      wire2.add(new WireSection(50,25,50,75));
      assertEquals("Upper right quadrant", new Point(50,50), findIntersections(wire1, wire2).get(0));
      wire1.clear();
      wire2.clear();

      wire1.add(new WireSection(25,-50,75,-50));
      wire2.add(new WireSection(50,-25,50,-75));
      assertEquals("Lower right quadrant", new Point(50,-50), findIntersections(wire1, wire2).get(0));
      wire1.clear();
      wire2.clear();

      wire1.add(new WireSection(-25,50,-75,50));
      wire2.add(new WireSection(-50,25,-50,75));
      assertEquals("Upper left quadrant", new Point(-50,50), findIntersections(wire1, wire2).get(0));
      wire1.clear();
      wire2.clear();

      wire1.add(new WireSection(-25,-50,-75,-50));
      wire2.add(new WireSection(-50,-25,-50,-75));
      assertEquals("Lower left quadrant", new Point(-50,-50), findIntersections(wire1, wire2).get(0));
      wire1.clear();
      wire2.clear();

      wire1.add(new WireSection(50,25,50,75));
      wire2.add(new WireSection(25,50,75,50));
      assertEquals("Upper right quadrant", new Point(50,50), findIntersections(wire1, wire2).get(0));
      wire1.clear();
      wire2.clear();

      wire1.add(new WireSection(50,-25,50,-75));
      wire2.add(new WireSection(25,-50,75,-50));
      assertEquals("Lower right quadrant", new Point(50,-50), findIntersections(wire1, wire2).get(0));
      wire2.clear();
      wire1.clear();

      wire1.add(new WireSection(-50,25,-50,75));
      wire2.add(new WireSection(-25,50,-75,50));
      assertEquals("Upper left quadrant", new Point(-50,50), findIntersections(wire1, wire2).get(0));
      wire1.clear();
      wire2.clear();

      wire1.add(new WireSection(-50,-25,-50,-75));
      wire2.add(new WireSection(-25,-50,-75,-50));
      assertEquals("Lower left quadrant", new Point(-50,-50), findIntersections(wire1, wire2).get(0));
      wire1.clear();
      wire2.clear();
   }

   @Test
   public void testFindClosestIntersection() {
      List<Point> intersections = new ArrayList<>();
      intersections.add(new Point(10, 10));
      intersections.add(new Point(-15, 5));
      intersections.add(new Point(-5, -5));

      assertEquals("Expected closest intersection", new Point(-5,-5), findClosestIntersection(intersections).get());
      Collections.reverse(intersections);
      assertEquals("Expected closest intersection", new Point(-5,-5), findClosestIntersection(intersections).get());
   }

   @Test
   public void testParseWire() {
      String testWireCode = "R75,D30,R83,U83";
      WireSection[] expected = {
              new WireSection(0,0,75,0),
              new WireSection(75,0,75,-30),
              new WireSection(75,-30,158,-30),
              new WireSection(158,-30,158,53)
      };

      assertArrayEquals("Test wirecode parsing", expected, parseWire(testWireCode).toArray());
   }

   @Test
   public void testWireSection() {

      // Test orientation
      WireSection wireSection1 = new WireSection(new Point(-1, -1), new Point(-99, -1));
      WireSection wireSection2 = new WireSection(new Point(-11, -11), new Point(-11, -1));
      assertEquals("Check orientation: ", Orientation.horizontal, wireSection1.orientation);
      assertEquals("Check orientation: ", Orientation.vertical, wireSection2.orientation);

      // Test contains
      WireSection wireSection3 = new WireSection(10, 10, 20, 10);
      assertTrue("Test positive contains 1", wireSection3.contains(new Point(15, 10)));
      assertEquals("Test expected length", 10, wireSection3.length);

      WireSection wireSection4 = new WireSection(10, 10, 10, 20);
      assertTrue("Test positive contains 2", wireSection4.contains(new Point(10, 15)));
      assertEquals("Test expected length", 10, wireSection4.length);

      WireSection wireSection5 = new WireSection(-10, -10, -20, -10);
      assertTrue("Test positive contains 3", wireSection5.contains(new Point(-15, -10)));
      assertEquals("Test expected length", 10, wireSection5.length);

      WireSection wireSection6 = new WireSection(-10, -10, -10, -20);
      assertTrue("Test positive contains 4", wireSection6.contains(new Point(-10, -15)));
      assertEquals("Test expected length", 10, wireSection6.length);

      WireSection wireSection7 = new WireSection(-10, -10, -20, -10);
      assertFalse("Test negative contains 5", wireSection7.contains(new Point()));
      assertEquals("Test expected length", 10, wireSection7.length);

      WireSection wireSection8 = new WireSection(-10, -10, -10, -20);
      assertFalse("Test negative contains 5", wireSection8.contains(new Point(-9, -19)));
   }

   @Test
   public void testFindFastestIntersection() {
      String testWireCode1 = "R75,D30,R83,U83,L12,D49,R71,U7,L72";
      String testWireCode2 = "U62,R66,U55,R34,D71,R55,D58,R83";
      List<WireSection> wire1 = parseWire(testWireCode1);
      List<WireSection> wire2 = parseWire(testWireCode2);
      List<Point> intersections = findIntersections(wire1, wire2);
      assertEquals("Test fastest intersection 1:",
              610,
              findFastestCombinedPath(intersections, wire1, wire2)
      );

      testWireCode1 = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51";
      testWireCode2 = "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7";
      wire1 = parseWire(testWireCode1);
      wire2 = parseWire(testWireCode2);
      intersections = findIntersections(wire1, wire2);
      assertEquals("Test fastest intersection 2:",
              410,
              findFastestCombinedPath(intersections, wire1, wire2)
      );

   }

   @Test
   public void testDistanceTo() {
      Point point1 = new Point();
      assertEquals("Test distance to 1", 10, point1.distanceTo(new Point(10, 0)));

      Point point2 = new Point(10, 0);
      assertEquals("Test distance to 2", 10, point2.distanceTo(new Point(10, 10)));

      Point point3 = new Point(-10, -10);
      assertEquals("Test distance to 3", 10, point3.distanceTo(new Point(-10, -20)));

      Point point4 = new Point(-10, -10);
      assertEquals("Test distance to 4", 10, point4.distanceTo(new Point(-20, -10)));

   }

   @Test
   public void testContains() {

   }

}
