...
public class Day3 {
    ...
    public static int solveStage1(String wireCode1, String wireCode2) {
        List<WireSection> wireSection1 = parseWire(wireCode1);
        List<WireSection> wireSection2 = parseWire(wireCode2);
        List<Point> intersections = findIntersections(wireSection1, wireSection2);
    ...
    private static Optional<Point> findClosestIntersection(List<Point> intersections) {
        return intersections.stream().filter(w -> w.x != 0 && w.y != 0).min(Comparator.comparingInt(point -> abs(point.x) + abs(point.y)));
    }

    private static List<Point> findIntersections(List<WireSection> wire1, List<WireSection> wire2) {
